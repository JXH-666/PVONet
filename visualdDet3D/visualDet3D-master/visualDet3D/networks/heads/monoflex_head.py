...
from visualDet3D.networks.heads.losses import IoULoss,GIOULOSS # GIOU
...
class MonoFlexHead(KM3DHead):
    ...
    def _L1Loss(output, target):
        loss = F.l1_loss(output, target, reduction='none') # smooth_l1_loss
        return loss.sum() / (len(output) + 1e-4)
    ...
    def _keypoints_depth_loss(self, depths, target, validmask, uncertainty):
        ...
        loss = F.smooth_l1_loss(depths, target.repeat(1, 3), reduction='none') * torch.exp(-uncertainty) + \
